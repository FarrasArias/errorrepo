...
def hinge_cost(m, a, b):
	...
	#return torch.mean((dist*(dist>0).float())**2)
...
def sim_metric_loss(self, model, inputs, return_outputs=False):
	...
	"""
	x_p = model(
		input_ids=inputs["x_p"], 
		sequence_lengths=inputs["x_p_sl"], 
		attention_mask=inputs["x_p_am"])
	x_n = model(
		input_ids=inputs["x_n"],
		sequence_lengths=inputs["x_n_sl"],
		attention_mask=inputs["x_n_am"])
	y_p = model(
		input_ids=inputs["y_p"],
		sequence_lengths=inputs["y_p_sl"],
		attention_mask=inputs["y_p_am"])
	y_n = model(
		input_ids=inputs["y_n"],
		sequence_lengths=inputs["y_n_sl"],
		attention_mask=inputs["y_n_am"])
	"""
