...
namespace util_protobuf {
	...
	midi::TrackFeatures* GetTrackFeatures(midi::Piece* p, int track_num) {
		...
		for (const auto track_num : track_nums) {
			...
			for (const auto bar_num : bar_nums) {
				...
				for (auto event_id : bar.events()) {
					e = p->events(event_id);
					if (e.velocity() > 0) {
						if (data_structures::is_drum_track(p->tracks(track_num).track_type())) {
							midi::Note note;
	...
	std::vector<midi::Note> track_events_to_notes(midi::Piece *p, int track_num, int *max_tick=NULL, bool no_drum_offsets=false) {
		...
		for (auto bar : p->tracks(track_num).bars()) {
			for (auto event_id : bar.events()) {
				e = p->events(event_id);
				if (e.velocity() > 0) {
					if (data_structures::is_drum_track(p->tracks(track_num).track_type()) && no_drum_offsets) {
						midi::Note note;
						note.set_start( bar_start + e.time() );
						note.set_end( bar_start + e.time() + 1 );
						note.set_pitch( e.pitch() );
	...
	*/
	...
	void random_perturb(midi::Piece* p, midi::Status* s, double per, int seed) {
		...
		auto pairs = link_events(p, arange(p->tracks_size()), arange(GetNumBars(p)));
	...
	void append_piece(midi::Piece* p, midi::Piece* q) {
		...
		int p_num_bars = GetNumBars(p);
		int q_num_bars = GetNumBars(q);
	...
	void UpdateMaxPolyphony(midi::Piece* p) {
		for (int i = 0; i < p->tracks_size(); i++) {
			int max_tick = 0;
			std::vector<midi::Note> notes = track_events_to_notes(p, i, &max_tick);
			GetTrackFeatures(p, i)->set_max_polyphony(max_polyphony(notes, max_tick));
		}
	}
	...
	void update_note_duration(midi::Piece* p) {
		for (int track_num = 0; track_num < p->tracks_size(); track_num++) {
			...
			GetTrackFeatures(p, track_num)->set_note_duration(note_duration_inner(notes));
		}
	}
	...
	void update_av_polyphony(midi::Piece* p) {
		for (int track_num = 0; track_num < p->tracks_size(); track_num++) {
			...
			GetTrackFeatures(p, track_num)->set_av_polyphony(
	...
	void prune_notes_wo_offset(midi::Piece* x, bool ignore_drums) {
		...
		for (const auto track : x->tracks()) {
			...
			if ((!ignore_drums) || (!data_structures::is_drum_track(track.track_type()))) {
				for (const auto bar : track.bars()) {
					for (const auto index : bar.events()) {
	...
	void UpdateValidSegments(midi::Piece* x, int seglen, int min_tracks, bool opz) {
		update_has_notes(x);
