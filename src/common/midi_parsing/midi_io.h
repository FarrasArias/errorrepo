...
#define QUIET_CALL(noisy) { \
    ...
    class PH {
    public:
        PH(std::string filepath, midi::Piece* piece, data_structures::EncoderConfig* config) {
        ...
        void parse(std::string filepath, midi::Piece* piece, data_structures::EncoderConfig* config) {
            ...
            if (ec->te) {
                util_protobuf::UpdateMaxPolyphony(piece);
        ...
        TT_VOICE_TUPLE infer_voice(int track, int channel, int inst, int pitch, data_structures::EncoderConfig* ec) {
            // special case for opz
            if (ec->te) {
                int track_type = (int)data_structures::TE_INST_MAP[inst];
                if (channel == DRUM_CHANNEL) {
                    track_type = (int)data_structures::TE_DRUM_MAP[pitch];
                }
    ...
    void ParseSong(std::string filepath, midi::Piece* p, data_structures::EncoderConfig* ec, std::map<std::string, std::vector<std::string>>* genre_data = NULL) {
        PH ph(filepath, p, ec);
    }

    void parse_te(std::string filepath, midi::Piece* p, data_structures::EncoderConfig* ec) {
        PH ph(filepath, p, ec);
    }
