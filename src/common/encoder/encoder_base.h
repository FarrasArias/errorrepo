...
std::vector<int> resolve_bar_infill_tokens(std::vector<int> &raw_tokens, REPRESENTATION *rep) {
  int fill_pholder = rep->encode(token::FILL_IN_PLACEHOLDER, 0);
  int fill_start = rep->encode(token::FILL_IN_START, 0);
  int fill_end = rep->encode(token::FILL_IN_END, 0);
...
public:
  ...
  */
  ...
  //#ifdef PYBIND
  py::bytes midi_to_json_bytes(std::string &filepath, data_structures::TrainConfig *tc, std::string &genre_data) {
    std::string x;
    midi::Piece p;
    midi_io::ParseSong(filepath, &p, config);
    util_protobuf::UpdateValidSegments(&p, tc->num_bars, tc->min_tracks, config->te);
