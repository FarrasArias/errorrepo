...
using TOKEN_TUPLE = std::tuple<token::TOKEN_TYPE,TOKEN_VARIANT>;
...
public:
  REPRESENTATION(std::vector<std::pair<token::TOKEN_TYPE,TOKEN_DOMAIN>> spec) {
    vocab_size = 0;
    for (const auto token_domain : spec) {
      token::TOKEN_TYPE tt = std::get<0>(token_domain);
  ...
  int encode(token::TOKEN_TYPE tt, TOKEN_VARIANT value) {
    std::tuple<token::TOKEN_TYPE,TOKEN_VARIANT> key = std::make_tuple(tt,value);
  ...
  int encode_partial(token::TOKEN_TYPE tt, TOKEN_VARIANT value) {
    auto it = token_domains.find(tt);
    if (it == token_domains.end()) {
      throw std::runtime_error("token::TOKEN_TYPE NOT PART OF THIS REPRESENTATION");
    }
    return it->second.encode(value);
  }
  int encode_partial_py_int(token::TOKEN_TYPE tt, int value) {
    auto it = token_domains.find(tt);
    if (it == token_domains.end()) {
      throw std::runtime_error("token::TOKEN_TYPE NOT PART OF THIS REPRESENTATION");
    }
  ...
  int get_domain_size(token::TOKEN_TYPE tt) {
  ...
  bool in_domain(token::TOKEN_TYPE tt, int value) {
    auto it = token_domains.find(tt);
    if (it != token_domains.end()) {
  ...
  bool is_token_type(int token, token::TOKEN_TYPE tt) {
    check_token(token);
    return std::get<0>(backward[token]) == tt;
  }
  token::TOKEN_TYPE get_token_type(int token) {
    check_token(token);
    return std::get<0>(backward[token]);
  }
  std::set<token::TOKEN_TYPE> get_mask_token_types(std::vector<int> &mask) {
    std::set<token::TOKEN_TYPE> tts;
  ...
  void show_mask_token_types(std::vector<int> &mask) {
    std::set<token::TOKEN_TYPE> tts = get_mask_token_types(mask);
    for (const auto tt : tts) {
      std::cout << toString(tt) << ", ";
    }
    std::cout << std::endl;
  }
  bool has_token_type(token::TOKEN_TYPE tt) {
    return token_domains.find(tt) != token_domains.end();
  }
  bool has_token_types(std::vector<token::TOKEN_TYPE> tts) {
    for (const auto tt : tts) {
  ...
  void set_mask(token::TOKEN_TYPE tt, std::vector<int> values, std::vector<int> &mask, int mask_value) {
    auto it = token_domains.find(tt);
    if (it != token_domains.end()) {
      for (const auto value : values) {
        if (value == -1) {
  ...
  void set_mask(token::TOKEN_TYPE tt, std::vector<std::string> values, std::vector<int> &mask, int mask_value, STRING_VECTOR_FLAG x) {
    auto it = token_domains.find(tt);
    if (it != token_domains.end()) {
      for (const auto value : values) {
        mask[encode(tt, value)] = mask_value;
  ...
  std::vector<int> encode_to_one_hot(token::TOKEN_TYPE tt, std::vector<int> values) {
    std::vector<int> x(vocab_size,0);
    set_mask(tt, values, x, 1);
    return x;
  }
  std::vector<int> get_type_mask(std::vector<token::TOKEN_TYPE> tts) {
  ...
  std::map<token::TOKEN_TYPE,int> domains;
  std::map<token::TOKEN_TYPE,TOKEN_DOMAIN> token_domains;
};
