...
public:
  ...
  std::vector<int> load_piece(size_t split_id, enums::ENCODER_TYPE et, data_structures::TrainConfig *tc) {
    ...
    util_protobuf::select_random_segment(&p, tc->num_bars, tc->min_tracks, tc->max_tracks, enc->config->te, &engine);

    enc->config->transpose = select_random_transpose(&p);
    return enc->encode(&p);
  }

  std::tuple<std::vector<int>,std::vector<int>> load_piece_pair(size_t split_id, enums::ENCODER_TYPE et, data_structures::TrainConfig *tc) {
    ...
    while (true) {
      try {
        ...
        util_protobuf::select_random_segment_indices(&p, tc->num_bars*2, tc->min_tracks, tc->max_tracks, enc->config->te, &engine, valid_tracks, &start);

        midi::Piece a(p);
        std::vector<int> abars = arange(start,start+tc->num_bars,1);
        util_protobuf::prune_tracks_dev2(&a, valid_tracks, abars);
       ...
       util_protobuf::prune_tracks_dev2(&b, valid_tracks, bbars);
  ...
  std::tuple<matrix<int>,matrix<int>> load_piece_pair_batch(int batch_size, size_t split_id, enums::ENCODER_TYPE et, data_structures::TrainConfig *tc) {
    enable_read();
    matrix<int> a;
    matrix<int> b;
  ...
  std::tuple<matrix<int>,matrix<int>> read_batch_v2(int batch_size, size_t split_id, enums::ENCODER_TYPE et, data_structures::TrainConfig *tc) {
    enable_read();
    std::unique_ptr<encoder::ENCODER> enc = getEncoder(et);
    if (!enc) {
      throw std::runtime_error("ENCODER TYPE DOES NOT EXIST");
    ...
    if (enc->rep->has_token_type(token::NUM_BARS)) {
      auto it = enc->rep->token_domains.find(token::NUM_BARS);
  ...
  std::tuple<matrix<int>,matrix<int>,tensor<double>> read_batch_w_feature(int batch_size, size_t split_id, enums::ENCODER_TYPE et, data_structures::TrainConfig *tc) {
    enable_read();
    std::unique_ptr<encoder::ENCODER> enc = getEncoder(et);
    if (!enc) {
      throw std::runtime_error("ENCODER TYPE DOES NOT EXIST");
  ...
  std::tuple<std::vector<std::vector<int>>,std::vector<std::vector<int>>> read_batch(int batch_size, size_t split_id, enums::ENCODER_TYPE et, data_structures::TrainConfig *tc) {
    enable_read();
    midi::Piece x;
    int index;
    int nitems = get_split_size(split_id);
    ...
    while (batch.size() < batch_size) {
      ...
      try {
        ...
        // pick random segment
        util_protobuf::select_random_segment(&x, tc->num_bars, tc->min_tracks, tc->max_tracks, enc->config->te, &engine);

        // pick random transpose
        std::tuple<int,int> pitch_ext = util_protobuf::get_pitch_extents(&x);
